...
def matchDesc(part):
    ...
    if part["oompType"] == "CAPC":
        desc = OOMP.getDetailsCategory("desc")
        match = False
        for test in desc:
            skips = ["SMD", ""]
            skip = False
            for s in skips:
                if s == test.name:
                    skip = True
            if test.name.replace(" ","") in description and not skip and not match:
...
def extraTags(part):
    ...
    part["extraTags"].append(OOMP.oompTag("oplPartNumber",{"code" : "C-JLCC", "name" : "JLC Parts Library", "partID" : part["LCSC Part"], "desc" : part["Description"]}))
    part["extraTags"].append(OOMP.oompTag("distributorPartNumber",{ "code" : "C-LCSC", "name" : "LCSC" , "partID" : part["LCSC Part"]}))
    part["extraTags"].append(OOMP.oompTag("manufacturerPartNumber",{ "code" : "C-XXXX", "name" : part["Manufacturer"], "partID" : part["MFR.Part"]}))
